...
def com_hillspot(
    aimd_path,
    xyz_path="XDATBUS_unwrapped.xyz",
    freq=100,
    ...
    hillspot_df = pd.read_csv(
        "HILLSPOT", delimiter="\s+", header=None, names=["cv", "height", "sigma"]
    )
    penaltypot_df = pd.read_csv(
        "PENALTYPOT", delimiter="\s+", header=None, names=["cv", "height", "sigma"]
    )
