...
def plot_fes(dat_file='fes_bias.dat', hills_file='HILLS'):
    ...
    dat = pd.read_csv(dat_file, sep='\s+', header=None, skiprows=skip_comments(dat_file))
    hills = pd.read_csv(hills_file, sep='\s+', header=None, skiprows=skip_comments(hills_file))

    fig = make_subplots(rows=1, cols=2, subplot_titles=("CV Plot", "FES Plot"))

    fig.add_trace(go.Scatter(x=hills[0], y=hills[1], mode='lines', name='CV'), row=1, col=1)
    fig.add_trace(go.Scatter(x=dat[0], y=dat[1], mode='lines', name='FES'), row=1, col=2)
